...
export class DevServerManager {
   ...
   * 自动启动开发服务器
   */
  async start(options?: DevServerOptions): Promise<string> {
   ...
   * 检测项目启动命令
   */
  private async detectStartCommand(cwd: string): Promise<string> {
   ...
   * 检测现有开发服务器
   */
  private async detectExistingServer(preferredPort?: number): Promise<string | null> {
    const ports = preferredPort
      ? [preferredPort, 3000, 3001, 5173, 5174, 8080]
      : [3000, 3001, 5173, 5174, 8080];

    // 去重
    const uniquePorts = [...new Set(ports)].filter((p): p is number => p !== undefined);
   ...
   * 等待服务器就绪
   */
  private async waitForServer(port: number, timeout: number): Promise<string> {
