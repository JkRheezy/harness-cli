import { RuleViolation, FixResult, RuleFix } from './types';
...
export class RuleFixer {
  private options: RuleFixerOptions;

  constructor(options: RuleFixerOptions = { dryRun: false }) {
  ...
  async fixFile(fileContent: string, violations: RuleViolation[]): Promise<FixResult> {
    ...
    const fixableViolations = violations.filter(
      (v) => v.autoFixable && v.fix?.replacement !== undefined
    );
  ...
  ): { success: boolean; original?: string; replacement?: string; reason?: string } {
  ...
  generateFixReport(violations: RuleViolation[]): string {
    ...
    lines.push('='.repeat(60));
    lines.push(
      `Summary: ${autoFixable.length} auto-fixable, ${manualFix.length} require manual fix`
    );
    lines.push('='.repeat(60));
   ...
   */
  countAutoFixable(violations: RuleViolation[]): { autoFixable: number; manualFix: number } {
    let autoFixable = 0;
    let manualFix = 0;
