...
exports.SmartTaskGenerator = void 0;
const Logger_1 = require("../../utils/Logger");
/**
 * 预定义模块的实现步骤映射
 */
const MODULE_IMPLEMENTATION_STEPS = {
    user: [
        '创建用户数据库模型',
        '实现注册/登录 API',
        '创建登录/注册页面',
 ...
 * 预定义模块的验收标准
 */
const MODULE_ACCEPTANCE_CRITERIA = {
    user: [
        '用户可以注册新账户',
        '用户可以使用邮箱/密码登录',
        'JWT token 正确生成和验证',
...
class SmartTaskGenerator {
     ...
     * @returns 实现步骤列表
     */
    generateImplementationSteps(gap) {
        // 首先检查是否有特定模块的预定义步骤
     ...
     * @returns 验收标准列表
     */
    generateAcceptanceCriteria(gap) {
        // 首先检查是否有特定模块的预定义标准
        const moduleName = this.extractModuleName(gap.name);
    ...
    generateDescription(gap) {
        ...
        if (gap.relatedFiles && gap.relatedFiles.length > 0) {
            lines.push(``, `## 相关文件`, ...gap.relatedFiles.map(f => `- ${f}`));
        }
        return lines.join('\n');
    }
    /**
    ...
    generateRequirements(gap) {
        ...
        switch (gap.type) {
            case 'missing_module':
                requirements.push('实现完整的功能模块', '包含必要的 API 和 UI', '添加适当的测试覆盖');
                break;
            case 'missing_api':
                requirements.push('实现 RESTful API 接口', '添加输入验证和错误处理', '编写 API 文档');
                break;
            case 'incomplete_flow':
                requirements.push('补全流程中的所有环节', '确保用户体验流畅', '处理边界情况');
                break;
            case 'missing_model':
                requirements.push('设计合理的数据结构', '实现 CRUD 操作', '添加数据验证');
                break;
            case 'config_mismatch':
                requirements.push('修复配置与实际代码的不一致', '确保配置正确加载', '更新相关文档');
                break;
        }
        return requirements;
    }
...
exports.SmartTaskGenerator = SmartTaskGenerator;
//# sourceMappingURL=SmartTaskGenerator.js.map