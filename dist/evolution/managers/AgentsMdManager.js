...
exports.AgentsMdManager = void 0;
...
class AgentsMdManager {
     ...
     * 解析表格行
     */
    parseTableRow(line, headers, defaultStatus = 'pending') {
     ...
     * 如果已存在则跳过
     */
    async addRequirement(gap) {
        const structure = await this.read() || this.createDefaultStructure();
        // 检查是否已存在
        const exists = structure.implemented.some(e => e.module === gap.name) ||
     ...
     * 记录完成时间
     */
    async markAsImplemented(moduleName) {
        const structure = await this.read() || this.createDefaultStructure();
        // 从待实现中移除并获取信息
     ...
     * 将模块标记为"进行中"
     */
    async markAsInProgress(moduleName, description) {
        const structure = await this.read() || this.createDefaultStructure();
        // 检查是否已在进行中列表
...
exports.AgentsMdManager = AgentsMdManager;
//# sourceMappingURL=AgentsMdManager.js.map