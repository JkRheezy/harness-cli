...
class AnalysisCoordinator {
    constructor(llmCaller, config) {
        ...
        // 初始化各个组件
        this.synthesizer = new Synthesizer_1.Synthesizer(this.llmCaller, this.config.models.synthesis);
        this.outputGenerator = new OutputGenerator_1.OutputGenerator(this.llmCaller, this.config.models.implementation);
     ...
     */
    async executeResearchPhase(options) {
        // 创建 4 个 Worker 实例
        const businessWorker = new workers_1.BusinessWorker(this.llmCaller, this.config.models.research);
        const techWorker = new workers_1.TechWorker(this.llmCaller, this.config.models.research);
        const domainWorker = new workers_1.DomainWorker(this.llmCaller, this.config.models.research);
        const riskWorker = new workers_1.RiskWorker(this.llmCaller, this.config.models.research);
     ...
     * @returns Worker 输出结果
     */
    async executeWorkerWithTimeout(worker, options, workerName) {
     ...
     */
    async generateAndVerify(synthesis, options) {
        ...
        for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
            ...
            try {
                // 阶段 3: 生成输出
                const analysis = await this.outputGenerator.generate(synthesis, options.projectName, options.overview);
                // 阶段 4: 验证输出
...
//# sourceMappingURL=AnalysisCoordinator.js.map