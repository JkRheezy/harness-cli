...
exports.MultiAgentPattern = void 0;
const path = __importStar(require("path"));
const PatternApplier_1 = require("./PatternApplier");
/**
 * Multi-Agent pattern implementation
 */
class MultiAgentPattern extends PatternApplier_1.BasePatternApplier {
    constructor() {
        super(...arguments);
        this.pattern = {
            name: 'multi-agent',
    ...
    canApply(input) {
        if (!input.patterns)
            return false;
        return input.patterns.some(p => typeof p === 'string' ? p === 'multi-agent' : p.name === 'multi-agent');
    }
    /**
    ...
    async apply(application) {
        ...
        try {
            ...
            return this.createResult(this.pattern.name, true, filesCreated, modifications);
        }
        catch (error) {
            return this.createResult(this.pattern.name, false, filesCreated, modifications, error instanceof Error ? error.message : String(error));
        }
    }
    /**
     ...
     */
    async createAgentTypes(srcDir, filesCreated, modifications) {
        const content = `/**
 * Agent Types
 * 
...
`;
     ...
     * Create BaseAgent abstract class
     */
    async createBaseAgent(srcDir, filesCreated, modifications) {
        const content = `/**
 * BaseAgent
 * 
...
`;
     ...
     */
    async createOrchestrator(srcDir, config, filesCreated, modifications) {
        const content = `/**
 * Agent Orchestrator
 * 
...
`;
     ...
     */
    async createMessageBus(srcDir, config, filesCreated, modifications) {
        const content = `/**
 * Message Bus
 * 
...
`;
     ...
     * Create agent implementations
     */
    async createAgentImplementations(srcDir, config, filesCreated, modifications) {
...
`;
     ...
     */
    generateAgentImplementation(agentDef) {
        return `/**
 * ${agentDef.name}
 * 
 * Role: ${agentDef.role}
...
exports.MultiAgentPattern = MultiAgentPattern;
//# sourceMappingURL=MultiAgentPattern.js.map