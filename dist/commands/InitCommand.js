...
const BusinessAnalyzer_1 = require("../services/BusinessAnalyzer");
...
class InitCommand {
    ...
    async execute(options) {
        ...
        if (!options.skipAnalysis && isInteractive) {
            const { enableSmartAnalysis } = await inquirer_1.default.prompt([
                {
                    type: 'confirm',
            ...
            if (enableSmartAnalysis) {
                // 输入项目概述
                const { overview } = await inquirer_1.default.prompt([
                    {
                        type: 'input',
                        name: 'overview',
                        message: '项目概述（一句话描述业务目标）:',
                        validate: (input) => input.trim().length > 10 || '请提供更详细的描述（至少10个字符）'
                    }
                ]);
                // 获取 LLM 配置
                const llmConfig = await this.loadLLMConfig();
                if (llmConfig) {
                    const analyzer = new BusinessAnalyzer_1.BusinessAnalyzer(llmConfig);
                    const spinner = (0, ora_1.default)('正在进行业务分析...').start();
                    try {
        ...
        try {
            ...
            // 生成智能文档
            if (businessAnalysis) {
                const docSpinner = (0, ora_1.default)('正在生成智能文档...').start();
                try {
            ...
            if (!options.autoStart && isInteractive && businessAnalysis) {
                const { startLoop } = await inquirer_1.default.prompt([
                    {
                        type: 'confirm',
                        name: 'startLoop',
                ...
                if (startLoop) {
                    console.log('\n🚀 正在启动自动化开发...\n');
                    const { LoopController } = await Promise.resolve().then(() => __importStar(require('../core/LoopController')));
                    const { ConfigLoader } = await Promise.resolve().then(() => __importStar(require('../utils/ConfigLoader')));
                    const config = await ConfigLoader.load(path_1.default.join(targetDir, '.harness/config.yaml'));
     ...
     * 生成智能文档
     */
    async generateSmartDocs(targetDir, analysis) {
        // 注册 Handlebars 辅助函数
        ...
        await fs_1.promises.writeFile(path_1.default.join(tasksDir, '001-initial-tasks.yaml'), tasksContent, 'utf-8');
    }
}
